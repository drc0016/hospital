...
const HospitalManagementSystem = () => {
  ...
  const fetchMisCitasHoy = async () => { if (user?.rol !== 'doctor') return; try { const r = await fetch(`${API_URL}/citas/mis_citas/?fecha=${new Date().toISOString().split('T')[0]}`, { headers: { 'Authorization': `Bearer ${token}` } }); const d = await r.json(); setMisCitasHoy(d.results || d); } catch {} };
